...
async function detail(inReq, _outResp) {
    ...
    for (const id of ids) {
        const resp = await request('http://live.yj1211.work/api/live/getRoomInfo?platform=douyu&roomId=' + id);
...
async function search(inReq, _outResp) {
    ...
    const data = await requestRaw(host + '/api/search/liveRoom?did=10000000000000000000000000001501&limit=40&offset=0&sk=' + wd, 'post');
    // const data = JSON.parse(resp.content);
    let videos = [];
    for (const vod of data.data.data.list) {
        videos.push({
            vod_id: vod.roomId,
